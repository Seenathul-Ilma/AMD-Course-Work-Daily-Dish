...
export const MealAPI = {
  ...
  searchMealsByName: async (query: string) => {
    try {
      const response = await fetch(
        `${MEAL_BASE_URL}/search.php?s=${encodeURIComponent(query)}`,
      );
  ...
  getRandomMeal: async () => {
    try {
      const response = await fetch(`${MEAL_BASE_URL}/random.php`);
      const data = await response.json();
      return data.meals ? data.meals[0] : null;
    } catch (error) {
  ...
  getCategories: async () => {
    try {
      const response = await fetch(`${MEAL_BASE_URL}/categories.php`);
      const data = await response.json();
  ...
  filterByIngredient: async (ingredient: string) => {
    try {
      const response = await fetch(
        `${MEAL_BASE_URL}/filter.php?i=${encodeURIComponent(ingredient)}`,
      );
      const data = await response.json();
      return data.meals || [];
    } catch (error) {
  ...
  filterByCategory: async (category: string) => {
    try {
      const response = await fetch(
        `${MEAL_BASE_URL}/filter.php?c=${encodeURIComponent(category)}`,
      );
      const data = await response.json();
      return data.meals || [];
    } catch (error) {
